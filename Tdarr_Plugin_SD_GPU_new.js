...
const details = () => ({
  id: 'Tdarr_Plugin_SD_GPU_new',
  Stage: 'Pre-processing',
  Name: 'Shane Dolley - Transcode to HEVC using Nvidia GPU & FFMPEG',
  Type: 'Video',
  Operation: 'Transcode',
...
const plugin = (file, librarySettings, inputs, otherArguments) => {
  ...
  for (let i = 0; i < file.ffProbeData.streams.length; i++) {
    ...
    if (codec_type === 'video') {
      ...
      if (
        ...
        response.processFile = false;
        response.infoLog += `File is already vp9 & in ${inputs.container}. \n`;
        response.preset = `<io> -map 0 -c:v hevc_nvenc -c:a copy -c:s copy -strict -2 -movflags faststart ${extraArguments}`;
  ...
  response.preset = `${genpts}<io> -map 0 -c:v hevc_nvenc -c:a copy -c:s copy -hide_banner -loglevel quiet -stats -strict -2 -movflags faststart ${extraArguments}`;
  response.processFile = true;
  response.infoLog += 'Transcoding. \n';
  return response;
};
